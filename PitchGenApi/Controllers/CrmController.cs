     ...
     [FromQuery] bool kraftedNotSent)
        ...
        public async Task<IActionResult> MatchContact([FromBody] ContactMatchRequestDto request)
        {
            ...
            var clientContacts = await (
                ...
                where df.client_id == request.ClientId &&
                    c.linkedin_url != null
                select new
                {
                    c.id,
            ...
            // SQL functions such as RTRIM/LOWER cannot be applied directly to text.
        ...
        public async Task<IActionResult> AddContactToDataFile(
            [FromBody] AddContactToDataFileRequestDto request)
        {
            if (request.ClientId <= 0)
                return BadRequest(new { message = "A valid ClientId is required." });
        ...
        [HttpPatch("EX_update-contact-fields")]
        public async Task<IActionResult> UpdateContactFields(
            [FromBody] UpdateContactFieldsRequestDto request)
        {
            ...
            var contact = await (
                ...
                join df in _context.data_files
                    on c.DataFileId equals (int?)df.id
            ...
            if (updatedFields.Count == 0)
                return BadRequest(new { message = "At least one field is required for update." });
