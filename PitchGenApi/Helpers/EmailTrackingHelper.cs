...
{
    public static string GetPixelTag(string email, int clientId, int? DataFileId,int? SegmentId, int contactId, string fullName, string location, string company, string website, string linkedin, string jobtitle, string trackingId)
    {
        ...
        return $"<img src=\"https://app.pitchkraft.ai/track/open?email={encodedEmail}&clientId={clientId}&SegmentId={SegmentId}&DataFileId={DataFileId}&contactId={contactId}&FullName={encodedName}&Location={encodedLocation}&Company={encodedCompany}&Website={encodedWeb}&linkedin_URL={encodedLinkedin}&JobTitle={encodedJob}&trackingId={encodedTrackingId}\" width=\"1\" height=\"1\" style=\"display:none; max-height:0; overflow:hidden;\" alt=\"\" />";
    }

    public static string InjectClickTracking(string email, string htmlBody, int clientId,int contactId, int? DataFileId,int? SegmentId, string fullName, string location, string company, string website, string linkedin, string jobtitle, string trackingId)
    {
        ...
        foreach (var link in links)
        {
            ...
            var trackingUrl = $"https://app.pitchkraft.ai/track/click?trackingId={encodedTrackingId}&email={encodedEmail}&url={encodedUrl}&clientId={clientId}&contactId={contactId}&DataFileId={DataFileId}&SegmentId={SegmentId}&FullName={encodedName}&Location={encodedLocation}&Company={encodedCompany}&Website={encodedWeb}&linkedin_URL={encodedLinkedin}&JobTitle={encodedJob}";
